...
public class NoAuthMongoClientWrapper implements MongoClientWrapper {
    ...
    try {
      ...
      try {
        ...
        if ( isPipeline ) {
          ...
          if ( Util.isEmpty( query ) && Util.isEmpty( fields ) ) {
            cursor = dbcollection.find().limit( numDocsToSample );
          } else {
            DBObject dbObject = (DBObject) JSON.parse( Util.isEmpty( query ) ? "{}" //$NON-NLS-1$
              : query );
  ...
  public static List<DBObject> jsonPipelineToDBObjectList( String jsonPipeline ) throws MongoDbException {
