...
/**
 * Created with IntelliJ IDEA. User: kwalker Date: 3/13/14 Time: 9:38 AM To change this template use File | Settings |
 * File Templates.
 */
public class MongoPropToOptionTest {
  ...
  public void testConfigureReadPref() throws MongoDbException {
    ...
    props.set( MongoProp.tagSet, TAG_SET );
    ...
    for ( String tagSet : tagSetTests ) {
      for ( String readPreferenceType : NamedReadPreference.getPreferenceNames() ) {
        props.set( MongoProp.readPreference, readPreferenceType );
        props.set( MongoProp.tagSet, tagSet );
        testReadPrefScenario( props, propToOption );
      }
    }
    // Test invalid READ_PREFERENCE
    props.set( MongoProp.readPreference, "Invalid" );
    try {
      propToOption.readPrefValue( props );
  ...
  public void testGetTagSets() throws MongoDbException {
    ...
    props.set( MongoProp.tagSet, tagSet2 );
    assertEquals( JSON.parse( tagSet2 ), wrapper.getTagSets( props )[ 0 ] );
    assertEquals( 1, wrapper.getTagSets( props ).length );



    props.set( MongoProp.tagSet, TAG_SET_LIST );
    ...
    props.set( MongoProp.tagSet, tagsAsArray );
    assertEquals( JSON.parse( "{ \"disk\": \"ssd\", \"use\": \"reporting\", \"rack\": \"a\" }" ),
      wrapper.getTagSets( props )[ 0 ] );
    assertEquals( 3, wrapper.getTagSets( props ).length );
    ...
    props.set( MongoProp.tagSet, tagSetInvalid );
    try {
